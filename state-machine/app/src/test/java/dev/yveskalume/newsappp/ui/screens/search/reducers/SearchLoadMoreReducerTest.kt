...
class SearchLoadMoreReducerTest : ShouldSpec({
    ...
    should("publish paging loading and next page events when paging is idle") {
        runTest {
            ...
            val result = reducer.reduce(state, SearchEvent.LoadMore)

            (result === state) shouldBe true
            events shouldBe listOf(
                SearchEvent.SetPagingLoading,
                SearchEvent.LoadArticles(page = 2)
            )
        }
    }
    ...
    should("do nothing when paging is not idle") {
        runTest {
            ...
            val state = SearchUiState(
                ...
                searchResultUiState = SearchResultUiState.Success(
                    news = FakeArticleRepositorySuccess.sampleArticles,
                    pagingState = SearchResultUiState.PagingState.EndReached
                )
            )
